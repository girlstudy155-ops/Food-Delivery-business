...
const API_BASE_URL = "https://food-delivery-business-production-00a9.up.railway.app/";
...
// ---------------- Cart Item Card ----------------
function CartItemCard({ item, theme }: { item: CartItem; theme: any }) {
  ...
  return (
    ...
    >
      {imageUri ? (
        ...
        <View
          style={[styles.itemImage, styles.itemImagePlaceholder, { backgroundColor: theme.border }]}
        >
          <Ionicons name="restaurant" size={24} color={theme.textSecondary} />
        </View>
      )}
      ...
      <View style={styles.itemContent}>
        ...
        <Text style={[styles.itemSize, { color: theme.textSecondary }]}>{item.size}</Text>
      ...
      <View style={styles.quantityControl}>
        <Pressable
          ...
          onPress={() => {
            ...
            // Always show minus; allow quantity to go to 1, do not remove automatically
        ...
        >
          <Ionicons
            name="remove"
            size={16}
            color={item.quantity > 1 ? theme.text : colors.textSecondary}
          />
        </Pressable>
...
export default function CartScreen() {
  ...
  let discount = 0;
  if (appliedCoupon) {
    const discountValue = safeNumber(appliedCoupon.discount_value || appliedCoupon.discount);
    if (appliedCoupon.discount_type === "percentage") discount = safeSubtotal * (discountValue / 100);
    else discount = discountValue;
  }

  const discountedSubtotal = Math.max(0, safeSubtotal - discount);
  const tax = discountedSubtotal * TAX_RATE;
  const total = discountedSubtotal + (items.length > 0 ? DELIVERY_CHARGE : 0) + tax;
  ...
  const applyCoupon = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}api/coupons/validate`, {
        ...
        body: JSON.stringify({ code: couponCode.trim().toUpperCase(), subtotal: safeSubtotal }),
      });
  ...
  if (items.length === 0) {
    return (
      <View style={[styles.emptyContainer, { backgroundColor: theme.background, paddingTop: topPad }]}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background, paddingTop: topPad }]}>
      <LinearGradient colors={["#FF4500", "#FF6B35"]} style={[styles.header, { paddingTop: 12 }]}>
        ...
        >
          <Ionicons name="trash-outline" size={18} color="rgba(255, 255, 255, 0.95)" />
        </Pressable>
      </LinearGradient>
      ...
      >
        <View style={styles.itemsSection}>
          {items.map((item) => (
            <CartItemCard key={`${item.product_id}-${item.size}`} item={item} theme={theme} />
          ))}
        </View>

        {/* Coupon Section */}
        <View style={[styles.card, { backgroundColor: theme.cardBackground, shadowColor: theme.shadow }]}>
          <Text style={[styles.cardTitle, { color: theme.text }]}>Coupon Code</Text>

          {appliedCoupon ? (
            <View style={[styles.appliedCoupon, { backgroundColor: theme.successBackground }]}>
            ...
            <View>
              <View style={styles.couponInput}>
                <TextInput
                  style={[styles.couponTextField, { backgroundColor: theme.border, color: theme.text }]}
        ...
        <View style={[styles.card, { backgroundColor: theme.cardBackground, shadowColor: theme.shadow }]}>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={[styles.summaryValue, { color: theme.text }]}>₹{safeSubtotal.toFixed(2)}</Text>
          </View>
      ...
      <View
        style={[
          ...
          { paddingBottom: Platform.OS === "web" ? 34 : insets.bottom + 90, backgroundColor: theme.cardBackground },
        ]}
      >
        <Pressable
